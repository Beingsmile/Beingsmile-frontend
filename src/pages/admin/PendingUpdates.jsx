...
import { FiCheck, FiX, FiClock, FiUser, FiLoader, FiMessageCircle, FiImage } from "react-icons/fi";
...
const PendingUpdates = () => {
  ...
  const reviewUpdate = useMutation({
    mutationFn: async ({ campaignId, updateId, status, rejectionReason }) => 
      await axiosInstance.post(`/admin/campaigns/${campaignId}/updates/${updateId}/review`, { status, rejectionReason }),
    onSuccess: (_, variables) => {
      qc.invalidateQueries(["pendingUpdates"]);
      toast.success(`Update ${variables.status === 'approved' ? 'approved' : 'rejected'}`);
    },
    onError: () => toast.error("Failed to review update")
  });
      ...
      <div className="grid gap-6">
        {pendingUpdates.length === 0 ? (
          ...
          pendingUpdates.map((item) => (
            <div key={item.updateId} className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col md:flex-row">
              {/* Sidebar Info */}
              <div className="w-full md:w-64 bg-gray-50/50 p-6 border-b md:border-b-0 md:border-r border-gray-100 space-y-4">
                ...
                <div className="space-y-1">
                  <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Submitted Date</p>
                ...
                <div className="flex gap-2 pt-2">
                   <button 
                     onClick={() => reviewUpdate.mutate({ campaignId: item.campaignId, updateId: item.updateId, status: "approved" })}
              ...
              <div className="flex-1 p-6 space-y-4">
                ...
                {item.images?.length > 0 && (
                  <div className="space-y-3 pt-3">
                     <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest flex items-center gap-2">
                        <FiImage size={12} className="text-primary" /> Attached Images ({item.images.length})
                     </p>
                     <div className="flex flex-wrap gap-3">
                        {item.images.map((img, i) => (
                          <a key={i} href={img} target="_blank" rel="noreferrer" className="relative group">
                            <img 
                              src={img} 
                              className="w-24 h-24 rounded-2xl object-cover border border-gray-100 group-hover:scale-105 transition-transform duration-300" 
